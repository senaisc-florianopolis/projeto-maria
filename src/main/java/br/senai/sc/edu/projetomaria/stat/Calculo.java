...
public class Calculo {
	...
	public void calcularMedia() {
		/*
		 * Caso o tamanho do Array seja menor do que o periodo de analise a
		...
		/*
		 * Caso o tamanho do Array seja igual o tamanho do periodo de analise, o
		 * calculo do erro quadrático médio nao é realizado.
		 */
		if (vendas.length == periodo) {
			eqm = 0;
		} else {
		...
		if (tipo == "M") {
			if (vendas.length > 1) {
				...
				int x = 0;
				/*
				 * Para cada posição do Array, utiliza-se uma variável para
				 * agregar os valores.
				 */
				...
				/*
				 * O resultado da concatenação é dividido pelo tamanho do array
				 * para se ter a média móvel do periodo recebido como parametro.
				 */
				resultado_media = media_venda
						/ (vendas.length - (vendas.length - periodo));
				...
				if (eqm == 1) {
					...
					for (int i = periodo; i < vendas.length; i++) {
						...
						diferenca += (vendas[i] - media_movel_dif)
								* (vendas[i] - media_movel_dif);
					}
			...
			if (tipo == "E") {
				...
				if (vendas.length > 1) {
					...
					for (int i = 2 + periodo; i < f.length; i++) {
						f[i] = alpha
								* vendas[(vendas.length - periodo - 1 + i)]
								+ (1 - alpha) * f[i - 1];
					}
					for (int i = 2; i < f.length; i++) {
						diferenca += (vendas[vendas.length - 12 + i] - f[i])
								* (vendas[vendas.length - 12 + i] - f[i]);
					}
					diferenca = diferenca / (f.length - 2);
					resultado = "Resultado Media Exponencial " + f.length
							+ " Meses: "
							+ String.format("%.2f", f[(f.length - 1)])
